  ...
  class Provider
    class Database
      class PostgresqlUser < Chef::Provider::Database::Postgresql
        ...
        def action_grant
          begin
            statement = "GRANT #{@new_resource.privileges.join(', ')} ON DATABASE #{@new_resource.database_name} TO #{@new_resource.username}"
            Chef::Log.info("#{@new_resource}: Granting access with statement [#{statement}]")

            db(@new_resource.database_name).query(statement)
            @new_resource.updated_by_last_action(true)
          ensure
            close
          end
