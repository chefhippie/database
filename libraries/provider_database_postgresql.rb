      ...
      class Postgresql < Chef::Provider
        ...
        private
          ...
          def db(dbname = nil)
            ...
            @db = ::PGconn.connect(
              :host => @new_resource.connection[:host] || @new_resource.connection["host"],
              :port => @new_resource.connection[:port] || @new_resource.connection["port"] || 5432,
              :dbname => @new_resource.connection[:database] || @new_resource.connection["database"],
              :user => @new_resource.connection[:username] || @new_resource.connection["username"] || "postgres",
              :password => @new_resource.connection[:password] || @new_resource.connection["password"]
            )
          end
