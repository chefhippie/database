    ...
    class Database
      class Mysql < Chef::Provider
        ...
        private
          def exists?
            begin
              ret = db.list_dbs.include?(@new_resource.database_name)
            ensure
              close
            end
          ...
          def db
            @db ||= begin
              ::Mysql.connect(
                @new_resource.connection[:host] || @new_resource.connection["host"],
                @new_resource.connection[:username] || @new_resource.connection["username"],
                @new_resource.connection[:password] || @new_resource.connection["password"],
                nil,
                @new_resource.connection[:port] || @new_resource.connection["port"] || 3306
              )
            end
          end
